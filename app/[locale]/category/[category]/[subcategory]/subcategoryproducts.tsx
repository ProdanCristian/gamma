...
export default function SubCategoryProducts({
  ...
  const productsUrl = useMemo(() => {
    return `/api/products/subCategoryProducts?subCategoryId=${subcategoryId}&page=${currentPage}&limit=12&minPrice=${
      priceRange[0]
    }&maxPrice=${
      priceRange[1]
  ...
  const maxPriceUrl = useMemo(() => {
    return `/api/products/maxPrice?subCategoryId=${subcategoryId}&bestsellers=${showBestsellers}&discounted=${showDiscounted}${Object.entries(
      selectedAttributes
    )
      .filter(([_, value]) => value !== "all")
      .map(([id, value]) => `&attr_${id}=${encodeURIComponent(value)}`)
      .join("")}`;
  }, [subcategoryId, showBestsellers, showDiscounted, selectedAttributes]);
  ...
  const queryKeys = useMemo(
    () => ({
      ...
      attributes: `/api/products/subCategoryFilters/attributes?subCategoryId=${subcategoryId}`,
      colors: `/api/products/subCategoryFilters/colors?subCategoryId=${subcategoryId}`,
      brands: `/api/products/subCategoryFilters/brands?subCategoryId=${subcategoryId}`,
    }),
    [productsUrl, maxPriceUrl, subcategoryId]
  );
