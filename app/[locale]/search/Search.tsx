...
export default function SearchClient({ translations }: SearchClientProps) {
  ...
  return (
    <div className="max-w-[1250px] w-[90vw] mx-auto my-10">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{translations.searchResults}</h1>
        {products.length > 0 && (
          <p className="text-gray-600 dark:text-gray-400">
            {translations.foundProducts.replace(
              "{count}",
              pagination.totalHits.toString()
            )}
          </p>
        )}
      </div>
      ...
      {loading ? (
        ...
        <p className="text-center py-4">{translations.noResults}</p>
      )}
